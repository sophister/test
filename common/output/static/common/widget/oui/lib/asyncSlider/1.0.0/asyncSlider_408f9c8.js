/**
 * Copyright © 2013, renrendai.com, Co,. Ltd. All Rights Reserved
 * Time: 2013.07.05
 * */
define('common:widget/oui/lib/asyncSlider/1.0.0/asyncSlider', ['require', "common:widget/oui/lib/jquery/1.9.1/jquery", "lib/easing/1.3.0/easing"],function(require) {
  var jQuery = require("common:widget/oui/lib/jquery/1.9.1/jquery"); 
  require("lib/easing/1.3.0/easing"); 
(function($) {
	var ct;
	var p = this;
	...
	function sliderPrepare(el, options) {
		if ( typeof ct == 'undefined') {
			...
			if ( typeof options == 'object') {
				...
				if ( typeof options.slidesNavType != 'undefined') {
					slides_nav_type = options.slidesNavType;
				}
			...
			ct.children("li").each(function(i) {
				var slide = $(this);
				var customSubElems = slide.find('[data-use]');
				var slide_elements = customSubElems.length === 0 ? slide.find('*') : customSubElems;
				slide.hide().css({
				...
				if (i === 0) {
					...
					bg_mask.css({
						...
						margin : 0,
						width : ct.outerWidth(),
						height : $(".ui-slide-inimask").height()
					});
					bg_mask_2.css({
						position : 'absolute',
						top : 0,
			...
			if (slides_nav) {
				...
				switch (slides_nav_type) {
					case "0":
						slides_nav_el.addClass("slider-nav-pointer");
						break;
					case "1":
						slides_nav_el.addClass("slider-nav-box");
						break;
					default :
						slides_nav_el.addClass("slider-nav-" + slides_nav_type);
				}
	...
	function sliderSetupNormally() {
		for (var i in slides) {
			var sld = slides[i];
			for (var j in sld.elements) {
				var yes_no = randomFromTo(0, i);
				sld.elements[j].fadeTo(0, randomFromTo(30, 90) / 100);
				if (yes_no % 2 == 0)
					sld.elements[j].html("");
			}
		}
		$("#asyncslider_next_prev_nav").css("top", randomFromTo(0, 100));
	}
	...
	function switchSlide(_in, _out) {
		...
		if (direction == 'horizontal') {
			...
			for ( i = 0; i < next_slide.elements.length; i++,slideStep = (i===4)?-50:slideStep+200) {
